...
- `noAccess` — нет прав на чтение файла;
- `invalidJson` — повреждённый или невалидный JSON;
- `failedToParse` — не удалось обработать содержимое;
- `invalidPath` — передан путь, который не ведёт к `package.json` или к папке пакета.
