...
`--dry-run` (`--dry`) — запускает команду в режиме симуляции. Показывает изменения, но не применяет их.

`--preid` `<id>` — задаёт кастомный префикс для преверсии (например, `alpha`, `beta.1`, `rc`).

`--skip-prompts` — пропускает интерактивные запросы. Используются значения по умолчанию.

`--skip-git` — не создаёт коммит и тег. Git-изменения остаются в рабочей директории.
...
`--skip-build` — пропускает выполнение `pnpm run build` перед публикацией.

`--skip-git` — отключает проверки git-состояния (аналог `--no-git-checks` в `pnpm publish`).
