...
- `scope`
      ...
      Задаёт соответствие названию аккаунта или организации в реестре NPM

- `scopeAsPackagePrefix`
      
      Включает преобразование путей к модулям, добавляя указанный `scope` перед названием пакета. По умолчанию - `false`

Пример `scopeAsPackagePrefix` для пакета `@myscope/globals`:

- Значение `false`
      
      Расположение `packages/globals`

- Значение `true`

      Расположение `packages/myscope-globals`
...
--dry
    ...
    Запускает команду в режиме симуляции ("dry run"), показывая изменения, которые будут произведены, но фактически ничего не меняя. Полезно для предварительного просмотра изменений перед применением.

--preid <custom-pre-release-id>

    Устанавливает кастомный префикс для предварительной версии, который добавляется к номеру версии пакета (например, beta.1). Эта опция позволяет создавать предварительные версии типа альфа, бета, RC и др. перед официальным стабильным выпуском.

--skipPrompts

    Пропускает интерактивные запросы пользователя. Команда выполняется автоматически, используя значения по умолчанию или заданные настройки.

--skipGit

    Игнорирует действия, связанные с системой контроля версий Git, такие как фиксация изменений, создание меток коммитов или отправка изменений на удалённый репозиторий. Может пригодиться, если вы хотите самостоятельно управлять операциями с Git позже.
...
--skipBuild
    ...
    Исключает запуск процесса сборки после обновления версий пакетов. Пропускает выполнение заданий, указанных в конвейере сборки, позволяя вам самим решать, необходима ли повторная компиляция после изменения номеров версий.
