...
export interface EnvLoaderOptions {
   ...
   * prefix: false
   * // → все переменные включаются (кроме учтённых в `keepNodeEnv`)
   *
   **/
...
/**
 * Разрешает пути к файлам окружения в зависимости от режима работы,
 * текущей директории и структуры репозитория.
 *
  ...
  for (const file of envFiles) {
    ...
    for (const variant of getEnvFileVariantsByMode(mode, file)) {
      ...
      if (envFilesVariants.has(variant) && !quiet) {
        ...
        console.warn(`[@mirta/env-loader] Duplicate env file "${variant}" generated by the "${file}" entry. Consider removing redundant entries from "options.envFile"`)
        continue
...
export function loadEnv(options: EnvLoaderOptions = {}) {
  ...
  const prefixes = options.prefix !== false
    ? ensureArray(options.prefix ?? [...DEFAULT_ENV_PREFIXES])
    : undefined

  if (prefixes) {

    // Фильтрация переменных, если задан префикс.
    processEnv = filterEnvKeys(processEnv, prefixes, keepNodeEnv)
