...
Пакет обеспечивает **консистентность** и **предсказуемость** на всех уровнях:

- **Инструменты разработки**, такие как `create-mirta` и `@mirta/cli`, сами собираются с помощью `@mirta/rollup/config-package`.
- **Генерируемые проекты** используют `@mirta/rollup/config` — обеспечивая единый стандарт для `wb-rules`.
- **Фреймворк Mirta** собирается теми же средствами — без исключений.
